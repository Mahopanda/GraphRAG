...
const ROW_LIMIT = parseInt(process.argv[3]) || 5; // 預設只處理5行
const DELAY_MS = parseInt(process.argv[4]) || 5000; // 預設5秒延遲

if (!DATA_FILE_PATH) {
  console.error("請提供資料檔案路徑");
  console.error(
    "使用方法: node load_custom_data_ultra_slow.js <資料檔案路徑> [行數限制] [延遲毫秒]"
  );
  console.error(
    "範例: node load_custom_data_ultra_slow.js my_data.csv 3 10000"
  );
  process.exit(1);
}
...
async function processChunksOneByOne(docId, textChunks, llmService) {
  console.log(
    `逐個處理 ${textChunks.length} 個文字塊，每個間隔 ${DELAY_MS}ms...`
  );
